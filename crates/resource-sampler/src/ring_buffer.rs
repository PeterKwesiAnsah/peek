// Rolling ring buffer for sparklines and FD leak detection.
//
// Logic moved from `peek-core::ringbuf` so multiple frontends can reuse it
// without depending on peek-core directly.
...
pub fn detect_fd_leak(
    ...
    let samples: Vec<u64> = history.last_n(window).iter().map(|s| s.fd_count).collect();
    if samples.len() < 3 {
        return None;
    }
